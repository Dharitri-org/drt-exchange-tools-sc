...
pub static OTHER_FARM_STAKING_TOKEN_ID: &[u8] = b"WMOAX-123456";
...
pub static DCT_ROLES: &[DctLocalRole] = &[DctLocalRole::Mint, DctLocalRole::Burn];

pub static SFT_ROLES: &[DctLocalRole] = &[
    DctLocalRole::NftCreate,
    DctLocalRole::NftAddQuantity,
    DctLocalRole::NftBurn,
];

pub static SFT_WITH_TRANSFER_ROLES: &[DctLocalRole] = &[
    DctLocalRole::NftCreate,
    DctLocalRole::NftAddQuantity,
    DctLocalRole::NftBurn,
    ...
    FarmStakingProxyObjBuilder: 'static + Copy + Fn() -> farm_staking_proxy::ContractObj<DebugApi>,
{
    pub fn new(
        ...
        b_mock.set_dct_local_roles(energy_dao_wrapper.address_ref(), LOCKED_TOKEN_ID, SFT_ROLES);

        let wrapped_locked_reward_token_roles = [DctLocalRole::Transfer];
        b_mock.set_dct_local_roles(
            energy_dao_wrapper.address_ref(),
    ...
    PairObjBuilder: 'static + Copy + Fn() -> pair::ContractObj<DebugApi>,
{
    ...
    b_mock.set_dct_local_roles(pair_wrapper.address_ref(), FARMING_TOKEN_ID, DCT_ROLES);
