    ...
    SimpleLockObjBuilder: 'static + Copy + Fn() -> energy_factory::ContractObj<DebugApi>,
{
    pub fn new(
        ...
        let router_wrapper = setup_router(
            &mut b_mock,
            &owner,
        ...
        b_mock.set_dct_balance(&first_user, MEX_TOKEN_ID, &user_balance);
        b_mock.set_dct_balance(&first_user, WMOAX_TOKEN_ID, &user_balance);

        b_mock.set_dct_balance(&second_user, MEX_TOKEN_ID, &user_balance);
        b_mock.set_dct_balance(&second_user, WMOAX_TOKEN_ID, &user_balance);
    ...
    PairObjBuilder: 'static + Copy + Fn() -> pair::ContractObj<DebugApi>,
{
    ...
    let lp_token_roles = [DctLocalRole::Mint, DctLocalRole::Burn];
    b_mock.set_dct_local_roles(pair_wrapper.address_ref(), LP_TOKEN_ID, &lp_token_roles[..]);

    pair_wrapper
}

fn setup_router<RouterObjBuilder>(
    b_mock: &mut BlockchainStateWrapper,
    owner: &Address,
    ...
    RouterObjBuilder: 'static + Copy + Fn() -> router::ContractObj<DebugApi>,
{
    ...
    b_mock
        .execute_tx(owner, &router_wrapper, &rust_zero, |sc| {
            sc.init(OptionalValue::Some(managed_address!(pair_address)));
            sc.pair_map().insert(
                PairTokens {
    ...
    FarmLockedObjBuilder: 'static + Copy + Fn() -> farm_with_locked_rewards::ContractObj<DebugApi>,
{
    ...
    let reward_token_roles = [DctLocalRole::Mint, DctLocalRole::Burn];
    b_mock.set_dct_local_roles(
        farm_wrapper.address_ref(),
        MEX_TOKEN_ID,
        &reward_token_roles[..],
